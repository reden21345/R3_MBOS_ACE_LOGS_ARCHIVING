...
# TABLES
export MBAudit=MB_AUDIT
export MBSyncLogs=MB_DAILY_SYNC_LOGS
export ApiLogs=API_LOG
...
# Temporary Files
export expdpTempLog=${TempDIR}/expdp_mbos.tmp 
...
export R3MBOSPW=`/usr/java8_64/jre/bin/java -jar $EncDecDIR/PasswordDecryptor.jar yKsbXwpFj45bwekTW+6uLMlwsj0pN4wr | awk '{print $3}'`


# Get Patition Names
export R3MBOSSchema=MBOSIUSR
export MBOSTables=${TempDIR}/mbos_tables.txt
${ORACLE_HOME}/bin/sqlplus -S /nolog <<  EOF > ${MBOSTables}
CONNECT "${R3MBOSUser}"/${R3MBOSPW}@${DBNAME} as sysdba
		...
		FROM (
			SELECT 
				DBMS_XMLGEN.GETXMLTYPE(
					...
					WHERE TABLE_OWNER = ''${R3MBOSSchema}'' 
					AND TABLE_NAME IN (
					''${MBAudit}'',
					''${MBSyncLogs}'',
					''${ApiLogs}'')'
...
export dtpmptbls=`cat ${MBOSTables} | grep .`
...
cat ${PARFILE} | egrep "DUMPFILE|TABLES" > ${mbosDump}
chmod 777 ${mbosDump}
